...
struct vertex
{
	...
	int idx;
	int visit = -1;
	long long int dist = 922337203685477580;
};
...
void dijkstra(int a)
{
	...
	while (!pq.empty())
	{
		...
		for (int i = 1; i < n + 2; i++)
		{
			...
			if (chkPrime(tmp_len) && vec[i].dist > tmp.dist + (long long int)tmp_len)
			{
				vec[i].dist = tmp.dist + (long long int)tmp_len;
				pq.push(vec[i]);
			}
		}
	}
...
int main()
{
	...
	if (vec[1].dist == 922337203685477580) cout << -1;
